...
here. Python FACTS are not repeated here either; they live in the Python set, whose entry point is
`README.md`: version and support facts in `python_platform_baseline_manifest.md`, type-system facts in
`python_typing_contract_manifest.md`, test-tooling facts in `python_testing_tooling_manifest.md`,
language hazards in `python_language_hazards_manifest.md`, the lint and practice catalogue in
`python_linting_practices_manifest.md`, the error contract in `error_tracing_contract_manifest.md`,
emission policy in `logging_observability_manifest.md`, live-process diagnosis in
`python_runtime_diagnostics_manifest.md`, module and packaging boundaries in
`python_module_boundaries_manifest.md`, execution models in
`python_concurrency_determinism_manifest.md`, and enforcement in `python_quality_gates_manifest.md`.
A spec cites those files for what is true; this file governs how the spec itself is written.

The open items this discipline produces (G5) are the same OPEN items the Python manifests raise: each
of them ends with a list of decisions the project must pin, addressed to this section.
